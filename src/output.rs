...
pub fn print_pull_requests(rows: &[PullRequestRow]) {
    ...
    for row in rows {
        let unresolved_comments = row
            .total_review_comments
            .map(|total| format!("{}/{}", row.unresolved_review_comments.unwrap_or(0), total))
            .unwrap_or_default();
        table.add_row([
            row.pr_number.map(|id| id.to_string()).unwrap_or_default(),
            row.status.clone().unwrap_or_default(),
            unresolved_comments,
            row.checks_status.clone().unwrap_or_default(),
            row.local_branch.clone().unwrap_or_default(),
            row.remote_branch.clone().unwrap_or_default(),
            row.repo.display().to_string(),
            row.worktree.as_ref().map(|p| p.display().to_string()).unwrap_or_default(),
...
pub fn print_repos(rows: &[(Repo, Option<String>, bool)]) {
    let mut table = table();
    table.set_header(["Repo", "Repo Origin", "Bare", "List PRs", "Setup", "Teardown"]);
    for (repo, origin, bare) in rows {
        table.add_row(vec![
            Cell::new(repo.path.display().to_string()),
            Cell::new(origin.clone().unwrap_or_default()),
            Cell::new(bare.to_string()),
            Cell::new(repo.list_pull_requests.to_string()),
